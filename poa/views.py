...
@login_required
def editar_proyecto_wizard(request, proyecto_id):
    """
    Configura la sesión del wizard para editar un proyecto existente en BORRADOR.
    Redirige al flujo del wizard.
    """
    ...
    if proyecto.estado != 'BORRADOR' and proyecto.estado != 'RECHAZADO':
        messages.error(request, 'Solo se pueden editar proyectos en BORRADOR o RECHAZADO.')
        return redirect('poa:lista_proyectos')
        
    # Inicializar sesión del wizard con este proyecto
    request.session['wizard_proyecto_id'] = proyecto.id
    request.session['wizard_paso'] = 1
